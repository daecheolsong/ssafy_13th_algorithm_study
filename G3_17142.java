...
public class G3_17142 {
	...
	// 0 빈칸, 1은 벽, 2는 바이러스
	static int[][] arr;
	static int n, m, answer = Integer.MAX_VALUE, virusCount;
	...
	public static void main(String[] args) throws IOException{
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				if (check && arr[i][j] == 0) check = false;
			}
		}
	...
	static void bfs() {		
		...
		while (!q.isEmpty()) {
			...
			if (arr[x][y] == 0) totalTime = Math.max(totalTime, time);
			for (int i = 0; i < 4; i++) {
				int cx = x + dx[i];
				int cy = y + dy[i];
				
				if (!isIn(cx, cy) || visit[cx][cy] || arr[cx][cy] == 1) continue;
